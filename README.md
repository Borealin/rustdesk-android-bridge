...
当前阶段：已有 Python 标准库实现的本机兼容性原型。本地 RustDesk 1.4.7 已显示 Pixel 8 的 H.264 视频及系统 SECURE 凭据页面；基本点击和拖动经用户真机复测通过，完整锁屏矩阵尚待验证。
...
本仓库是独立项目。RustDesk、scrcpy 等上游源码尚未引入；后续接入时记录准确提交或发布版本，并保留适用许可证。公开仓库仅保留脱敏后的验证摘要；原始日志、屏幕截图、设备标识和个人会话记录不随项目分发。
...
当前固定 H.264、最大边长 1280、最大 30 fps、4 Mbps；没有音频、剪贴板同步、文件传输、ID 服务、编码切换或完整键盘映射。当前 `PeerInfo` 按 Android/1.4.7 兼容字段发送，仅表示实验目标协议，不代表完整实现原版 RustDesk 功能。
