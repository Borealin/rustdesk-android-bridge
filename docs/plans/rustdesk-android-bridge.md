...
macOS 可为独立桥接进程指定物理网卡，绕过 TUN 对原生 TCP/UDP 地址和端口复用的影响，保留原 RustDesk 加密协议。不改系统全局路由或代理规则，配置只放私有 runtime。NAT 探测及一次局域网加密直连已观察通过，跨蜂窝网络仍允许中继兜底，更多网络矩阵待验证。接口切换需要更新配置，API HTTP/WebSocket 不属于此 socket 绑定范围。
