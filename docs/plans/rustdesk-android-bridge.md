...
区分 chr/unicode 字符、seq 文本提交、控制键和目标 Android 物理键码。ASCII 可打印字符使用 scrcpy 字符注入；中文/emoji 等使用手机剪贴板粘贴，会覆盖手机剪贴板，禁止粘贴的字段不保证可用。分离按下/抬起的键跟踪状态并在断线释放。完整硬件布局和复杂输入法时序仍待验收。
...
macOS 可为独立桥接进程指定物理网卡，绕过 TUN 对原生 TCP/UDP 地址和端口复用的影响，保留原 RustDesk 加密协议。不改系统全局路由或代理规则，配置只放私有 runtime。NAT 探测及一次局域网加密直连已观察通过，但随后 iOS 蜂窝连接持续卡在登录前，尚不能宣称中继兜底稳定。现场已移除接口配置并启动网络改动前的构建；该基线下 Mac 原版客户端经同一中继完成加密登录、收到视频并发送输入，而 iOS 仍超时。用户随后确认强杀 iOS 客户端后 Wi-Fi 可连、蜂窝仍不可连；当前保持基线运行，待定位蜂窝路径的登录阶段超时后再启用接口优化。接口切换需要更新配置，API HTTP/WebSocket 不属于此 socket 绑定范围。
